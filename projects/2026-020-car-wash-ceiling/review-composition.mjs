...
const failures=[];
try{
 for(const short of (process.argv.includes('--main-only')?[false]:[false,true])){
 ...
 const shortShots=short?JSON.parse(fs.readFileSync(path.join(P,'01_script/shorts-scene-plan.json'))).shots:[];
 const times=short?[0,.6,...shortShots.map(s=>s.start+Math.min(.5,(s.end-s.start)/2)),sd-4,sd-.2]:data.shots.map(s=>s.start+Math.min(1,(s.end-s.start)/2));
 const images=[];
 for(const [i,t]of times.entries()){
 await page.evaluate(t=>{Object.values(window.__timelines)[0].seek(t,false);},t);
 const bounds=await page.evaluate(()=>[...document.querySelectorAll('.statement,.map-panel,.caption,.short-hook,.short-cta')].filter(e=>getComputedStyle(e).visibility!=='hidden'&&Number(getComputedStyle(e).opacity)>.05&&e.getBoundingClientRect().width).map(e=>({class:e.className,x:e.getBoundingClientRect().x,y:e.getBoundingClientRect().y,right:e.getBoundingClientRect().right,bottom:e.getBoundingClientRect().bottom})));
...
fs.writeFileSync(path.join(P,process.argv.includes('--main-only')?'05_review/layout-main-check.json':'05_review/layout-check.json'),JSON.stringify({status:failures.length?'fail':'pass',failures},null,2));
