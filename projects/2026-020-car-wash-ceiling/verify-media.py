...
decoded=subprocess.run(['ffmpeg','-hide_banner','-i',str(video),'-vf','freezedetect=n=-55dB:d=3,blackdetect=d=1:pix_th=0.05','-af','volumedetect','-f','null','-'],capture_output=True,text=True,check=True)
log=decoded.stderr
(P/f'05_review/logs/{fmt}-media.log').write_text('\n'.join(line.rstrip() for line in log.splitlines())+'\n')
freeze=re.findall(r'freeze_duration: ([0-9.]+)',log)
black=re.findall(r'black_duration:([0-9.]+)',log)
freeze_starts=re.findall(r'freeze_start: ([0-9.]+)',log)
freeze_ends=re.findall(r'freeze_end: ([0-9.]+)',log)
if len(freeze_starts)>len(freeze_ends):
    trailing=float(v['duration'])-float(freeze_starts[-1])
    if trailing>3:freeze.append(str(trailing))
assert not freeze,freeze
assert not black,black
peaks=re.findall(r'max_volume: ([0-9.-]+) dB',log)
assert peaks and float(peaks[-1])<0,peaks
