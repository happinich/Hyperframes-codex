...
for shot in shots:
    at=shot['start'];number=int(shot['visual'].split('-')[-1])
    if number in [6,11,21,22,48]:
        for i in range(3):event(at+.4+i*.8,'step',.15,.007)
    if number in [16,24,33,34,39]:event(at+.2,'scratch',.6,.009)
    if number in [35,42]:
        for i in range(6):event(at+.3+i*.43,'step',.22,.014)
    if number==47:event(at+.1,'notification',.45,.006)
def save(name,data):
...
(P/'02_audio/sound-design.json').write_text(json.dumps({'seed':19,'voice_unchanged_during_mixing':True,'bgm_candidate':'cand-01','selection_basis':'User delegated atmospheric selection; final choice subject to measured mix review.','ambience':'procedural fluorescent room hum and soft air','events':events,'bgm_gain_db':-18,'outro_seconds':4,'fade_out_seconds':3},ensure_ascii=False,indent=2)+'\n')
