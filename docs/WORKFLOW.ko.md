...
본편·쇼츠·썸네일 모두 `config/success-rules.json`의 `image_review_rules`를 적용합니다. 생성 직후 각 이미지를 원본 크기로 확인하고, 렌더 전에는 선택된 모든 이미지를 대본·앞뒤 장면과 대조하여 한 번 더 검수합니다. 축소 모음이나 파일 검사만으로 통과시키지 않습니다.

손·신체 형태, 사물 구조와 접촉, 인물·귀신·차량의 연속성, 공간과 진행 방향, 대사와 등장 시점, 의도하지 않은 얼굴·문자, 모바일 단서 가독성을 확인합니다. 의도된 초자연 연출과 생성 오류는 구분합니다.

이상하거나 판단이 불확실한 컷은 영상에서 제외하고 프롬프트를 수정해 재생성합니다. 교체 이미지도 두 검수를 다시 통과해야 합니다. 재시도 횟수만 채웠다는 이유로 미해결 오류를 허용하지 않습니다. 이미지별 파일/해시, 두 검수 결과, 오류와 재생성 이력, 최종 판정을 `05_review/image-review.md`에 기록합니다. 최종 출력에서도 크롭·확대·모션 때문에 새 문제가 생기지 않았는지 확인합니다.
...
쇼츠 화면은 본편에서 추출하지 않습니다. 별도 대본·음성에 맞춰 세로 이미지도 새로 생성합니다. 본편 이미지 재사용, 프레임 추출, 크롭은 금지합니다. 생성 프롬프트·출처와 자산 해시를 기록하고 본편 자산이 섞이지 않았는지 검수합니다.
