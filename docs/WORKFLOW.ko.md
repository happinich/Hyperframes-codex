...
공포 프로필에서는 설명형 규칙을 그대로 사용하지 않습니다. 첫 5~10초에 가장 소름 돋는 대사 한 줄을 선제시한 뒤 장소에 간 이유와 정상 상태를 구체적으로 앉히고, 40~60초 사이에 첫 이상 징후를 보여줍니다. "지금 화면을 보시면" 같은 메타 안내 멘트는 몰입을 깨므로 금지합니다. 새 대본은 `했다체`를 기본으로 하며, 한 영상에서 6~8분 이야기 한 편을 완결합니다. 귀신·괴물 축은 흔적, 부분 노출, 직접 상호작용 순으로 존재를 확인시킵니다. 세부 기준은 [`HORROR_CHANNEL_STRATEGY.ko.md`](HORROR_CHANNEL_STRATEGY.ko.md)를 따릅니다.
