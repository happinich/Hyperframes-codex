...
- 콜드 훅 뒤 장소와 정상 규칙을 앉히고 40~60초 사이에 첫 이상 징후를 보여줍니다.
- 사람이 경험을 직접 들려주는 존댓말 구술형 이야기체로 6~8분 이야기 한 편을 완결합니다.
- `~했어요`, `~였죠`, `~거든요`, `~더라고요`, 필요한 `~했습니다`를 섞고 했다·였다 단문을 연속하지 않습니다.
- 귀신·괴물 축은 흔적, 부분 노출, 직접 상호작용 순으로 존재를 명확히 확인시킵니다.
- 본편은 클린 마스터와 SRT/VTT가 기본이며 화면 자막본은 별도 승인된 경우에만 만듭니다.
